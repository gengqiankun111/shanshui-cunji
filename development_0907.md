...
>
> 2026-09-07 状态回填治理（逐项核对代码/config 现状）：P0-B/P1-C/Task-025 已收口销项（历史保留）；
> Ex-9.3⑤ 载荷默认化 / Ex-8.12 L2 压缩默认化 / Task-002 三处"似完成实未排"按代码核实如实标注；
> M-7 缺口按 P131 落地收窄；P141 行已注接线完成。回填明细见各对应行内「2026-09-07 治理」注。
...
> 🔶 P93 实验（2026-09-05，see problem_solving P93）：#29 110 万 PAX 实测（PAX 列仍在整块内 → 整块
> 解压 ≈ 行式 IO，PAX 无 IO 收益，串行 13~19s）；并行分片 top-K（select.rs，按 docid 等分子窗独立堆
> → 全局合并）语义单测通过（746 全绿），但 110 万实测：块缓存 put 速率超淘汰 → RSS 超预算膨胀至
> ~9.7G（非泄漏：空闲稳态 5.2G 收敛于配置上限）+ 多核利用率低（~2/12 核）→ **默认关闭**
> （P93_PARALLEL=1 显式开启实验）。内存判定：4G 块缓存配置下私有内存 5349MB 稳态不涨 = 缓存按
> 配置填满即停，非泄漏。**#29 达线未达成（状态保持 ⏳）**；下一步候选 P94：块缓存写穿/淘汰节流
> （scan 路径 put 速率限速）+ 文件内并行分块扫描 或 列分块 SST（免整块解压）后再启并行。
> 📌 **P94 立项（2026-09-05 用户确认：热列旁路双轨，行式主副本 + hot_fields 热列列存旁路）**
> 方向：列分块双轨唯一根治全表排序瓶颈；**不整表双写**（全 25 列双写=写放大×2 伤写侧卖点），
> 行式 SST 为主（全列/点查/SELECT * 无雪崩），仅 `hot_fields` 热列（k/amount/ts/…）以**独立块/
> 独立文件**旁路列存（引擎级配置决定，客户零感知=方案 A 的 CX 红利；在线格式升级/灰度/一键回退行式
> = 方案 A 的运维红利）。列分组由引擎 hot_fields 决定，客户无需建表语法。
> 阶段：① 热列（k/amount）独立块写入+读取，topk 稠密/投影路由列存 → **#29 110 万 ≤1.5s 达线先行**
> （预估 30~50MB 列 IO、<0.5s，比 P93 整块解压 ~500MB 减 ~90%）；② 双轨路由 + EXPLAIN 标注
> （TableScan: Columnar/RowStore）+ 回退开关；③（视需）全列列存化/列族。
...
> ✅ 收口（2026-09-07 治理）：① 分区剪枝经 P100 profile 判定现状已具备（文件/层/表/块四级剪枝 + P62/P3-A/P1-E + 数值列块级 zone 安全化落地），非剪枝缺口，#5 小窗主因 = 固定 seek+块 IO+宽行返回 → 转并行/块 IO；② 并行扫描随 Task-025b ①②③④（P102/P103/P104/P109：窗口并行全扫聚合 → 通用 WHERE 并行 + GROUP BY 分片合并 → Engine::scan_range_parallel → CF scan_stream_at_parallel 跨文件扇出）全部落地，数值收益面向多 L0 大文件窗口随基准轮回填。本项无剩余开发。
...
- Task-002：fxhash 局部替换（内部 Key）——真实缺口 ✅（Cargo.toml 全库无 fxhash）、验收（内部 Key 延迟 ≥15%）可控 ✅、工作量 1 天 ✅ → **执行**。**2026-09-07 治理核实：仍未动工（Cargo.toml 无 fxhash）；P96 会话曾评估"读已极快"边际小 → 搁置未排期**
...
| **Task-005 回填（2026-09-05，两档全量 81 探针 vs MySQL）** | results/results-pax-sqlrun-compare-{100k,1100k}/summary.md（SCC cfg = tmp-cfg-wide-2g-pax，**未启 colstore**）。100k：#75 update_range_idx 26×、#73 14×、排序族 4~5×、#60 group_by_two_where ~2.4×；1100k：**#75 223.9×（582ms vs MySQL 2.6ms，放大爆炸）**、#73 10.7×、排序族 24~27×（PAX 无 colstore，行式地板；colstore cfg 专项 #29 已 0.23s 反超，见 P124）、#60 5.3×、#77 txn_long_read 19×（p99 抖动）、#58 sum_where_idx 4.0×、#62 count_distinct_highcard 2.1×、#63-66 深排序 14~27×、insert/delete/组合索引/事务档位 0.0~1.5× 全优。**结论分层：写链（P88/P89 管道）正常；慢点 = ①条件定位（#75 组合 WHERE+LIMIT，见下 P127）②行式全扫/整行解码（排序族/聚合）③长快照窗输出** | 回填 ✅（记录于 results/，见基准记录）。**110 万复测（2026-09-06 部分跑通，P127/P128/P129/P130 后）**：MySQL 3316 侧全量 81 完成（results-pax-sqlrun-compare-1100k-mysql2）；SCC 3317 侧全量**连环写态 8h 僵死**（23:28 倒排 seg32 后无活动，rr CPU 0——P122 类机制性风险，重启复现前单探针全健康）→ 改**定向复测**（results/retest-scc-*）：读/本日改动侧 #2 点查 1.05×、#5 pk_between_100 **169ms→11ms**（早停收敛生效）、#41 500 行 39.7ms、#43 全窗 10000 行 734ms（宽表 10000 行整 doc 回表解码地板）、#6 倒排正常；写风险组（#17/73/75/24/74/72）**全部完成无卡死**——#75 干净态 21ms vs **连环写后 120ms**（update/upsert 链 memtable 累积 → keys-only 定位跨 memtable 退化，运行中写链状态，区别于 P130 open 场景——候选机制：写链 memtable 驻留期间定位扫描成本，另行评估）；#74 delete1000 8ms、#72 upsert500 17.9ms、#24 0.42ms。**后续 → ✅ 干净库全量复测完成（2026-09-06，results/results-pax-sqlrun-compare-1100k-clean/，SCC 分 10 批全 81 探针）**：SCC 干净重建 110 万（wide-load 74s）后分批跑通无卡死（污染库态两次 8h 异常 = 多轮写/段态下极慢扫描，非死锁——cmp_between 干净态 4.1s vs 污染态 8h）。**#75 update_range_idx：223.9× → 3.5×（mean 10.5ms vs MySQL 3.01，≤10× 验收 ✓ P127 闭环）**；#5 pk_between_100 0.8×（反超）、点查族 0.1×、倒排族 0.0~2.5×、#24 delete_range50 0.1×、批量写/事务族 0.0~2.6× 全优。#41/42/43 pk_between 500/3000/10000 = 7.1/8.2/8.3×（宽表整行回表地板）；**剩余 >10×（已知族）**：排序 #63-66 18.6~31.2×、#29 24.5×（PAX 无 colstore 行式地板，colstore cfg #29 已反超见 P124）、#77 txn_long_read 16×（p99 8.7s 抖动）、#56 like 15.7×、#13 sum_where 10.6×（守卫回退）；UPDATE 家族 #73 6.5/#18 4.0/#17 2.9/#19 3.0×（仍 >update_id ≤2× 触发线，维持"规避 UPDATE"决策）；扫描族 #11 6.3/#55 6.9/#54 7.2/#53 9.9× | ✅ 干净库全量复测 + **P134-137 后 110 万复测（2026-09-06，results/p137-pax-scc-1100k + 合表 p137-pax-compare-1100k，归档 user_guide/性能对比-2026-09-06-P134-137后-110万.md）**：当日新 binary（P134 事务读快照语义收口/P135 batch_get_at+txn 倒排 superset/P136 预过滤/P137 监控）81/81 全过 0 失败无卡死；比值与 clean 轮同族一致——点查/COUNT/索引/写链/事务档位 0.0~1.5×（#35 RR 0.8×、#36 0.7×、#26 0.6×、#78 0.0× → P134-135 RR 路径无回归）；慢点仍为既有已知族（宽表整行回表 #41-43 9.8-15.3×、倒排大窗 #45/46/48 12-27×、排序族 #29/63-66 31-72×、like #56 21×、长快照窗 #77 37×、sum 守卫回退 #13 38×） |
| **P127（2026-09-05 触发立项：UPDATE 与组合 WHERE 定位专项）** | 实测定性（110万 PAX 库 rr --one 拆解）：`UPDATE … WHERE id BETWEEN 1..20000 AND status='active' LIMIT 200` 582ms；**同条件只读 SELECT = 499ms**（写链增量仅 ~12ms/200 行 → P89 管道无 bug）；纯 `status='active' LIMIT 200` = 0.89ms（倒排快）、纯 `id BETWEEN 1..20000 LIMIT 200` = 169ms；**组合形态 ~0.5s 恒定（窗口 4k/20k/100k 不变）→ 组合 WHERE(BETWEEN∩等值) 未走倒排∩区间收敛/早停，疑似全候选（active posting）逐行求值到 LIMIT**，随数据量爆炸（100k 62ms→1100k 582ms ≈ 9.4×）。**开发候选（排期评估）**：①组合条件收敛——BETWEEN 区间与等值 posting 先做 DocIdSet 交集再消费（LIMIT 早停 P85 已具）；②单字段 UPDATE delta patch（免整行读/重写/重索引）；③UPDATE/INSERT 落盘等待对齐。验收：#75 110万 ≤ MySQL 10× 内、update_id ≤2× | 立项（profile demo 先行）→ **✅ 已完成（2026-09-05，P127，见下行）** |
| **P127 ✅ 落地（2026-09-05，组合 WHERE 定位收敛）** | profile demo（src/demo/p127-combo-where，20 万行 active 50%）拆出两慢点：① **写定位全候选遍历**（`id BETWEEN ∩ status='active'`，get_docid_set limit=None → post_filter 遍历全 active posting：20 万 52ms → 110 万 ≈550ms 线性 9.4×）；② **composite 前缀命中全候选复筛**（声明 `[status]` 后组合 SELECT LIMIT 200 = 181ms@20 万 → 110 万 ≈500ms 且窗口恒定——用户实测 SELECT 499ms 的根因，无 composite 版组合 SELECT 仅 0.09ms 早停）。**落地**：① server 写定位（UPDATE/DELETE 字段/复合路径）新增三 helper（sqlparse.rs）：`strip_where_limit`（剥离 LIMIT n → 只更新/删除前 n 命中行并定位早停，修复 UPDATE ... LIMIT 被忽略全量写的语义偏差）、`extract_pk_between_comb`（纯 AND 链提取主键 id/docid BETWEEN 叶，OR/NOT 保守回退通用求值）、`locate_pk_range_converged`（row 闭区间 → docid 区间 **keys-only** ∩ 其余条件 DocIdSet.contains，LIMIT 命中即停）；② executor composite 路由守卫：WHERE = 等值 + 主键区间组合 → 跳过 composite 前缀回退 eval 收敛（纯主键区间/单列主键 composite 范围路由不受影响）。**验证**：3 新增单测（UPDATE/DELETE 组合 LIMIT 截断只动前 n 行 + 无 LIMIT 全量不截断、SELECT 守卫行序=升序前 200 active）；lib 772 通过 + 3 ignored（seqlock 概率型 flaky 单跑复绿，无关）。**基准复测待办**：#75（110 万）写定位 ~550ms → 区间 keys-only（2 万行）级、UPDATE 全链 ≤ MySQL 10× 验收回填（需 3317 重启 + rr sqlrun --only update_range_idx） | ✅ 已完成（2026-09-05，P127）；**110 万验收复测 ✅（2026-09-05）**：3317 重启新 binary 实测 #75（results/p127-verify-scc|mysql，同库同轮）——mean 117.4ms / p50 46ms（--one 稳定 32ms，纯主键区间版 21.5ms）vs 修复前 582ms；MySQL 对照 mean 5.63ms / p50 3.36ms → 倍数 223.9× → mean 20.8× / p50 ≈10.7~13.7×；rows=200 ✓（LIMIT 生效对齐 MySQL，原 UPDATE ... LIMIT 被忽略全量写已修）。**SELECT 组合主键语义 bug（残余①）→ 分支 B 已修（2026-09-05）：根因 = doc 无 id 字段（INSERT id 列提取为主键不进 JSON）→ executor 字段语义求值 `id BETWEEN` 恒 0 行慢查；修复 = `execute_with_tid`（server select_response 传表号）+ executor 主键区间收敛（`pk_range_select`：纯 AND 提取 row 区间 → docid 区间 keys-only ∩ 其余条件集 → 回表 LIMIT/OFFSET，排序需全候选不收敛保守回退）；110 万复测组合 SELECT：0 行 + 12s → **200 行、p50 16.1ms**（MySQL 同 SQL p50 0.49ms → 33×，750× 提速；残余 = 宽表 200 行整 doc 回表解码，归"行式解码/回表"家族）；新单测（真实形态 doc 无 id：组合 LIMIT/无 LIMIT 1000/OFFSET/纯区间，lib 775 通过）。**另发现独立慢点**：纯主键区间 SELECT（server extract_between_range 快速路径）110 万 p50 296ms（历史 169ms 同源）——该窗口路径远慢于组合收敛路径（16ms），建议复用 pk_range_select（rest=None 快路径）列为后续小项；**残余②（UPDATE affected 对齐）→ 已修（2026-09-05，6fb2a07）**：update_response 字段级与 doc 整体替换分支——变换后新值==旧值（赋值同值）**跳过写**（MySQL affected 0 语义；免整文档重写+倒排重索引→同键版本堆积→#75 第二轮 p50 46→98ms 逐轮退化已消除，多轮 rows=0 稳定）；单测 update_same_value_skips_write_affected_zero（0/50/0 + 值断言）；lib 776 通过。delta patch → **✅ 升格 P131 立项（2026-09-06，用户选定方案：每列 ≤1 delta + MVCC seq，设计 research/delta-patch.md）→ P131b 已落地（379bad4，词条声明过滤微优化，用户复核选向）**：server 写路径 doc_terms 只生成倒排**声明字段** term（inverted/fulltext/stats/bitmap_fields；引擎 `term_index_fields`），未声明列（note/city）不再构造+提交 term（省 CPU/字典/段膨胀）；实测值变 UPDATE 200 行写链 **32→21-23ms（-34%）**；#75 官方探针因 loader note 同值恒 rows=0 = **读现值路径**（SCC p50 8.7-11ms vs MySQL 4.1-10ms ≈1-2×，同值跳过已生效）——"写链③"真实成本须值变形态测（如上 21ms）；wrapper 内嵌方案因 base+p 字节 ≥ 整 doc（写字节不减）**暂缓**，保留设计供 delta-only+跨版本合并（引擎 Merge 语义）评估时参考。单测 p131b_doc_terms_declared_fields_only；lib 779 通过。**另注（已定位 2026-09-05）**：修复后 #75 复测绝对 p50 ~190ms / 组合 SELECT 160ms / 纯主键区间 760ms 的临时退化 = **重启 WAL 回放 121 万全量驻留 memtable（只读不触发 flush）**——期间窗口扫描跨 memtable+段合并慢 10×；写 1 行触发 flush(114 万)+compact 后全面恢复：组合 SELECT p50 **18.8ms**、#75 **21.1ms（rows=0，MySQL p50 2.93 → 7.2× ≤10× 验收 ✓）**。衍生小项 → **✅ P130 已做（2026-09-05）**：CF `memtable_over_threshold` 判定（mutable ≥ max_size，与 maybe_flush 同判据）+ engine open 收尾（worker/组提交启动前）primary/delta/cidx 超阈即 `switch_and_flush`（空/未超阈不刷防空 L0）。重启实测：队列回放 123 万后 open 阶段即主动 flush 114 万落盘、先于监听启动——只读服务不再驻留大 memtable（此前需预热写触发）。单测 p130_memtable_over_threshold_flag；lib 777 通过。稳定独立慢点 = server `extract_between_range` 纯主键区间快速路径 ~360ms（组合收敛路径 19ms 反快），复用 pk_range_select（rest=None）小项保留 |
| **P131 集成 ✅（2026-09-06，用户选定 Delta CF 路线）** | SQL 单非索引列 `SET col=字面量` → `Engine::patch_batch` 字段增量（免整行重写/免重索引）；**统一增量 MVCC 版本规则**（用户 2026-09-06 约束：倒排索引与一切 RR 查询必须 MVCC——增量只对 ≥ 其 seq 快照可见；设计 research/delta-patch.md §10/§11，wrapper 初案标已弃用参考）。**落地**：①写——dml update_response delta/全量分流（声明配置 + 列 ∉ term/组合/保留、非自增、base 在 → patch_batch；同值跳过 affected 0）；②读——Delta Merge-on-Read 补全缺口（scan_range/paged/after/stream/fields/zonepred/parallel + txn scan_range_txn RR 按 seq 门控；`delta_overrides_range(_at)` + fold helpers；无增量 `data_empty()` 短路零开销）；③修复既有隐患 get_at 增量多版本坍缩（scan_raw_range_with_seq 先最新坍缩→快照丢旧补丁；改 CF scan_stream_at ≤snapshot 归并）。4 新增单测；lib **782 通过 + 3 ignored**。详见 problem_solving P132 | ✅ 已完成（2026-09-06，P132）；**值变形态 #75 压测回填 ✅（2026-09-06，clean 110 万 pax 库 db-wide-scc-pax，结果 results/p131-delta-scc2|scc3|scc-full|mysql）**：note 逐轮异值（rows=200/轮 ×20）——SCC Delta 稳态 p50 **21.3ms**（mean 21.1~22.1，p99 ≤27）；同库 nodelta（无声明→全量重写）p50 **35.3ms**（p99 77）→ **Delta 省 ~40%（p50 1.66×，p99 尾收敛 3.4×）**；MySQL 同探针 p50 2.96ms → 值变 p50 ≈7.2×（P131 前同形态历史 p50 46ms ≈13.7×，减半）；官方同值 rows=0 读现值 SCC p50 6.35ms vs MySQL 3.57 ≈1.8×。**写链残余 = 定位/折叠读/提交常数**（21ms ≈ locate ~6ms + 200 行读 + 组提交 fsync），未达 update_id ≤2× 触发线 → 维持触发式（delta 之上需再收定位/提交常数）→ **✅ 已收（2026-09-06 追加：定位 v2 + 批量组提交化）**：`commit_batch`（批量写按档位 0/2 走组提交 ack，档 1 显式 flush 不变）+ `live_window_ids`（live 位图∩窗口纯内存定位，替换 keys-only 20000 键磁盘扫；>200k 跨度回退不劣化）。**干净 110 万端到端（results/p131-final-scc|scc2）：#75 官方 rows=0 p50 6.35→1.3ms（反超 MySQL 3.57）；值变 rows=200 p50 21.3→7.6ms（≈2.6× MySQL）**。单测 +1（live 窗口==keys-only 口径：删行/limit/复活）；lib **783 通过** |
| **P129（2026-09-05，per-table L0 优先压实）** | 多表（split_by_table，每段单表）下全局 L0 阈值 [8,12] 与表数强耦合（每批 flush 产出 N 表文件，20 表单批 20 段 ≥ 阈 → 每写必**全量** L0 合并 / 调高阈值致热点表失控）。**落地**：config `storage.per_table_l0_trigger`（默认 2）；CF 压实决策改按表口径（compaction.rs）——`per_table_active`（split_by_table + 阈值>0 + L0 含 ≥2 表且无混表段；单表库/旧混表段自动回退全局 = 零回归）+ `table_l0_subset`（目标表 = L0 段数最多且 ≥ 阈值的表，输入其 L0 段合并去重 → 输出 **L1 新段**，不并入既有 L1——单次压实量有界防无界历史重写；L1 同表多段由 L0 空时 bottom split 收敛）+ `needs_compact` 多表分支（主触发 = 存在表 ≥ 阈；全局计数不作触发——每表受控 ⇒ 全局有界）。**语义**：稳态每表 L0 ≤1 段（点查 O(1) 段），每批 flush 每表一次小合并（写放大 ≈2×，对比全量多倍）；20 表各 1 段 = 收敛态（no-op，旧逻辑 20>12 会全量）。**验证**：2 新单测（热点表 t1 两批仅合并 t1 且 t7 留 L0 不参与、第 3/4 批 flush 后 L0 t1 2 段再合并且 L1 不并入 + 跨批覆盖读正确；20 表 × 1 段 needs/compact 均 no-op + 读回）；全量 lib **774 通过 + 3 ignored** | ✅ 已完成（2026-09-05，P129）；**监控指标补充同日 ✅（见 problem_solving P129）** |
| **内存口径观测（2026-09-05 触发，2G 预算 vs RSS 4.47G）** | 实测（110万 PAX 库运行态 WorkingSet = **4,469 MB**，配置预算 hotcache1024+bc512+inv256+mem256 = 2G payload）。认知修正：**cache payload 配额 ≠ 进程 RSS 上限**（元数据/倒排临时 bitmap/读 buffer/碎片/mmap 另计）。已有历史锚点：P93/P124（4G blockcache 配置稳态 RSS≈5.35G 不涨 = 缓存填满即停，非泄漏；9.7G 为并行扫描瞬时超淘汰）。**待做三组观测（只读基线 / 读写混合 / 同组探针重复看单向上涨）+ cache 元数据纳入统计**；候选优化（评估后立项）：倒排查询临时 bitmap 复用/及时释放、memtable 跳表行打包降元数据、cache 统计含元数据。容量规划乘 1.4~1.8× | 观测待排（独立会话） |
| **Bloom 读路径核查（2026-09-05，audit 结论见 research/bloom_readpath_audit.md）** | **布隆已全链实现并接线（非缺失）**：v5 分区布隆（每数据块一个，fpr 默认 0.01）+ v3/v4 legacy 兼容；flush 与 compaction 合并输出均由 SstWriter 重建新布隆（merge.rs 传 bloom_fpr）；读路径（当前读/快照读/批量点查）均在读块前调用目标块 `maybe_contains`（read.rs 351/361/406/454 等）。886ms max 长尾非布隆缺失（冷 IO/首触 + L0 多段线性放大更可能）。**残余缺口进度**：①Bloom 分层计量（**已完成 2026-09-05**：CF BloomCounters 六计数——minmax_skip/legacy_skip/part_probe·skip·pass/fp_est，get/get_many/get_at 埋点；`/metrics` gauge `shanshui_bloom_*` + SHOW MEMORY；引擎级冒烟测试，lib 765 通过。剩余：L0 4/8/12/16 + miss 点查压测脚本用该计数验证收益）；②每 SST bloom/索引元数据字节纳入 memory_report；③MVCC 快照生命周期管控（已落地，见下行）；④886ms 尖峰冷热对照实验 | audit ✅；计量 ✅；②/④ 待排（L0 分段压测脚本待排） |
| **P128（2026-09-05，P0 观测三项补齐——Bloom 分层/块缓存计数/L0 按表段数）** | ① **块缓存 (hits/misses/evicts) 计数**：BlockCache 原子计数 + `cache_stats()`（CF `blockcache_stats` 透出）→ engine `blockcache_report` gauge `shanshui_blockcache_{hits,misses,evicts}_total`（三 CF 聚合）；② **Bloom 过滤计数按层分桶**：`BloomCounters` 单桶 → `layers: [BloomLayerCounters; 3]`，读路径（get/get_at/get_many）按所在层埋点；导出 18 行 `shanshui_bloom_{...}_total_{l0,l1,l2}` + 既有 6 总口径保留（`/metrics` 与 SHOW MEMORY 行集扩展；metrics.render 与 engine_runtime_gauges 改 String 化以支持动态名）；③ **per-table L0 段数 gauge**：CF `l0_table_counts`（遍历 L0 层按 `table_id` 计数，split_by_table 每段单表前提）→ engine `l0_table_report` `shanshui_l0_sst_count_table_{tid}`——多表热点"全局 L0 未满但单表 L0 堆积"观测，为 per-table L0 优先压实调度提供输入。**顺带修正两认知**：filter/索引不进 blockcache（常驻 SstReader）→ "被 LRU 淘汰"不成立，blockcache 只缓存数据块；L0 元数据 open 即常驻 → "L0 元数据冷 IO"不成立（仅 L2 full_index 首访懒加载一次）。4 新增单测（blockcache 计数、分层落桶 L0→L1、CF 冷热读计数、多表 L0 按表）；全量 lib 769 ✅。**待办 → ✅ 实测回填（2026-09-05，P128 压测）：L0 段数 4/8/12/16 × miss 点查（demo `src/demo/l0-bloom-scale`，release，偶数 8000 行交错写 S 段 + 奇数 miss 5000 次）：probe/q = 4.00/7.99/11.99/15.98 ≈ 段数（直证逐段遍历线性）；p50 1.20/1.80/2.60/3.50 µs、mean 1.57/2.10/3.06/4.18 µs（斜率 ≈0.21µs/段）；pass/q（fp）0.036→0.13 ≈ fpr 0.01 ✓；blockcache misses/q≈0 → miss 开销全在每段 二分+布隆反序列化（CPU/内存），非磁盘块读。**决策**：① **per-table L0 优先压实 → 立项（上调高优先，2026-09-05 用户修正）**——本压测仅证单表 miss 读放大绝对斜率 ≈0.21µs/段（命中 key 早停不受影响）；多表结构面另有关键失效：split_by_table 每批 flush 产出 N 表 × 每表 1 个 L0 文件，全局 L0 有效阈值 [8,12]（clamp max16）与表数**强耦合**——20 表单批 flush=20 段 ≥ 阈值 → 每写必触发**全量** L0 合并（全部表参与，写放大爆炸）；为规避调高全局阈值又令热点表段数失控（读放大）。需 per-table 独立口径：某表 L0 段数达每表阈值（如 2~4）只压该表段下沉 L1，其余表不参与；全局计数仅作最坏兜底（见 P129 排期）；②分层 fpr 调优对纯 miss 无意义（真 miss 无 fp）→ 仅命中场景假阳性读块才受益，需另测命中档，暂不立项；③候选"分区布隆反序列化每查询现做"量级 ≈0.2µs/段 不单独立项 | ✅ 已完成（2026-09-05，P128） |
...
### 倒排 MVCC/RR 专项排期（2026-09-06 立项；同日机制澄清修订：posting 不加版本号，snap_seq 为准）

> 设计依据：research/inverted-versioning-rr.md。**用户确定读法**：走倒排索引时**倒排内不加版本号**
> （posting 维持最新态候选、只加不删）；读取顺序 = 先取全局 MVCC seq（快照 S）→ 再读 term→docid
> 候选 → 逐 docid 以 snap_seq 到主表裁决（可见性 + 字段值），**所有数据以 MVCC snap_seq 为准**。
> 范围：P134 正确性收口先行；P135 快照读统一接线（demo 先行）；C/D 随阶段；B 排序不做独立项。

| 项 | 内容 | 状态 |
|---|---|---|
| **P134 事务读快照语义收口（正确性）✅（2026-09-06，见 problem_solving P134）** | ① `txn_select_by_predicate` 字段谓词 RR 快照读漏行（候选=最新态 sqlish，快照后并发删/换值行不在候选）→ 改为**快照窗口分块扫描**（`txn_select_by_predicate_snapshot`：scan_range_txn 逐窗 + doc_matches_where 复检 + LIMIT 早停 + 自写补入；仅默认表单行域 watermark≤2^48 启用，RC/FOR UPDATE/混合库回退旧路径）；② `scan_range_txn` 删除位图剔除改仅 snapshot=MAX（RC/当前），RR/Serializable 跳过由 tombstone seq 裁决（get_at 一致）；③ 核对结论：快照锚定 = BEGIN（`txn_begin` begin_snapshot=global seq-1），MySQL RR = 首条一致性读——差异真实、影响面大暂不改，记录已知边界；顺带 inverted `doc_count`/`search_paged` mem Vec sort+dedup（merge_distinct 升序前提）。**核实撤销（2026-09-06）：另记录的两"缺口"（autocommit 纯 id 投影返空 / 快速连插后全扫漏首行）经引擎级 + 服务 fn 级双决定性验证 = 测试解析假象**（row_ids 按 2 列结果集头偏移 skip(4)，`SELECT id` 为 1 列头包偏移不同 → 丢首行/单行全丢），非引擎/投影缺陷，撤销记录 | ✅ 已完成（2026-09-06，P134）；2 新集成测试（谓词 RR delete/update 轮、BETWEEN SUM 快照后删不变）；全量 lib 784 通过（seqlock flaky 单跑复绿） |
| **P135 倒排快照读统一接线 ✅（2026-09-06，见 problem_solving P135）** | `Engine::batch_get_at(docids,S)` ✅（语义=逐 get_at：跳过位图、tombstone seq 裁决、Delta ≤S 折叠单次窗口；不入 HotCache）；**txn 谓词 superset ✅**——RR 字段谓词 WHEN 纯倒排等值 AND 链 → `inverted_eq_superset`（posting 只加不删 = ever-match 超集）∩窗口 → `batch_get_at(S)` 复核 + `doc_matches_where`（恢复索引加速、免 P134 全窗扫）；复杂/空 posting/含未提交写 → 全窗扫兜底。**其余端复核决策（不改）**：非事务倒排消费端已全链 latest 批量（P2-D/P85）且 autocommit 无 RR 义务（语句内无并发写 → S=语句开始≡latest），改 `batch_get_at(S=now)` 纯退化（≤S 归并 + sst_min_seq 冷段全键扫）→ **选型 = 保留 latest 批量位图快路径**；区间/组合同理（autocommit 保持，RR 端由 scan_range_txn 窗口/superset 覆盖）。`batch_get_at` 消费点 = RR 快照语义端（txn superset）+ 未来混合库 superset 残余 | ✅ 已完成（2026-09-06，P135）；3 单测/集成（batch==get_at 引擎测、superset RR 端到端、superset gating）；全量 lib 787 过（seqlock flaky 复绿） |
| **P136 快照活跃预过滤 ✅（2026-09-06，见 problem_solving P136）** | `Engine.snapshot_dels`（docid→删除提交 seq）事件表：写路径 位图+per-CPU 模式记账（delete/delete_batch 记 del、复活 put 清除、purge 复位；open 空表——活跃快照不跨进程故免基线）+ `prune_deleted_before_snapshot(ids,S)`（集合级剔"删于 ≤S 且未复活"候选，**绝不误剔**，历史删→复活窗口/非维护模式漏剔由 batch_get_at None 兜底）+ 接线 P135 `finish_predicate_superset` 批量前 prune | ✅ 已完成（2026-09-06，P136）；单测 p136_snapshot_dels_prune（S0/S1/S2 × 复活 × per-CPU/位图开关 4 组合）；全量 lib 789 过 |
| **P137 倒排专项监控（D）✅（2026-09-06，见 problem_solving P137）** | `Engine.inverted_report` 9 行 gauge：倒排段数/内存 docid 深度/段落盘 counter（新增 seg_flush_total）/posting 位图缓存命中·未命中（新增 counter，query 缓存路径埋点）/GC 与 delta FST 超限待收敛标志（1/0）+ 快照读 batch docid 累计与预过滤剔除累计（engine 新增 snapshot_batch_rows/snapshot_prefilter_saved，batch_get_at/prune 埋点）——接入 `/metrics`（engine_runtime_gauges）与 SHOW MEMORY（query.rs show_memory_response）双链 | ✅ 已完成（2026-09-06，P137）；单测 p137_inverted_report_rows_and_snapshot_counters（9 行存在 + cache miss→hit + prune/batch 计数前进）；全量 lib 790 过 |
| **P138 autocommit S 选型 demo ✅（2026-09-06，见 problem_solving P138）** | P135 复核结论"非事务端不改（保留 latest 批量位图快路径）"基于 S=语句开始≡latest（autocommit 语句内无并发写）**定性**。本项：**定量 A/B demo**——非事务倒排/谓词回表 `Engine::batch_get_at(S=语句级全局 seq)` vs 现 `batch_get`（latest+位图）直连引擎对比（位图开/关 × memtable 热/冷 × 大窗/小窗），量化 ≤S 归并（get_bytes_at vs get_bytes）+ Delta 窗口 + HotCache 旁路三项成本差 → 定论"不改"或产出触发项（若冷段 sst_min_seq keys-only 首触等场景退化显著） | ✅ demo 完成（2026-09-06，src/demo/p138-autocommit-s，gitignored）：60k 行 s=a 候选 30k × 位图开关 × memtable 热(留 mem)/冷(已 flush SST) × 窗 30000/4096/512，`batch_get_at(S)` **全 8 单元均慢于 latest batch_get 1.6–2.4×**（debug 相对口径：latest 1.07–2.30 µs/docid vs S 2.56–2.96；位图开/关与热/冷均成立，冷段无 keys-only 首触放大）——≤S 归并 + HotCache 旁路（warm 后 latest 命中回填）成本实锤 → **维持"非事务端不改"**（S 路径为 RR 专属），无触发项 |
| **P139 投影/回表瘦身（首切片）✅（2026-09-06，见 problem_solving P139）** | 收"已知性能族"之**投影/回表瘦身**首切片：执行器两纯字段回表消费端——`pk_range_select`（主键区间）与非 sort 通用分支（倒排候选/组合候选/IN 列表分块消费 `collect_limited_rows`）在 **SELECT 纯字段列集**（无 `*`/表达式/别名，且至少一个非 id/docid 字段；id-only 保守回退整行）时改 `batch_get_fields` 只解所需列（PAX 列解码 / 行式按需字段提取，P86②/P87 基建复用），子集 JSON 组装输出（缺字段省略）；新增 helper `select_projection_fields/subset_doc_bytes/batch_fetch_rows` | ✅ 已完成（2026-09-06，P139）：单测 p139_projection_subset_matches_full（pk 区间与倒排两路径 × 子集==整行基线 k/status 等值 + note 不出现 + 墓碑剔除一致）；lib 791 绿。**clean 110 万实测回填（3317 db-wide-scc-p139，wide-load 67s 后立即跑，MySQL 对照取 results-pax-sqlrun-mysql-1100k）**：大窗候选稀疏族收益显著——#46 enum_sel_limit10000 **240→58ms（4.1×，MySQL 8.91 → 6.5×）**、#48 combo_and_limit3000 **43.7→7.4ms（5.9×，MySQL 3.11 → 2.4×）**、#45 enum_sel_limit3000 37.6→12-18ms（2-3×）；#47 combo_and_limit500 1.51→1.18；pk 区间 #43 176→170（±小）、#05 p50 稳定 1.9-2.1ms（同态 A/B：fields 333 vs 整行 541ms @ dirty state，投影仍优）；小窗 500 行级（#44/49/50）受热缓存/噪声波动（0.6-2.3ms，与 P137 同量级）。**形态定论**：整行 25 列解码 > fields 逐键开销，稠密/稀疏窗口均投影更优；id-only/含 `*` 保持整行（墓碑剔除语义依赖）。**下一片**：同族残余 = **id-only 活跃判定 ✅（2026-09-06，P139-c）**——`Engine::batch_alive_latest`（位图 O(1)/墓碑模式存在性点查；前置契约=候选源自现存集）+ 三输出端（pk 区间/非 sort 通用/topk）纯 id 时免整行输出 `{}`；实测同库 #47 combo_and_limit500 1.18→0.68ms、**#48 combo_and_limit3000 7.39→1.58ms（4.7×，对 MySQL 3.11 反超 0.5×）**；单测 p139b_batch_alive_latest_matches_get + p139 路径⑤；lib 792 绿。**再剩**：sort 分支输出投影（P139-b ✅）、txn 长快照窗快照投影流（#77）、id-only 其余端、#79/80 归 colstore 族 |
| **P140 快照投影流（#77 txn 长快照窗）demo-first（2026-09-06 立项）** | 前置核对 ✅：#77 `txn_long_read` = `SELECT id,k,amount … WHERE id BETWEEN a AND a+100000`（**3 列投影，非 SELECT ***）→ 投影下推价值成立。现状：事务长读窗走快照读路径（scan_range_txn 逐窗整 doc 25 列 + ≤S 版本归并），未受 P139 影响；脏态实测 mean 10.7s/p99 40s（状态退化，需干净基线）。**方案**：引擎/CF 新原语"快照 + 列投影"——`scan_range_txn_fields(start,end,S,fields)`（跨 memtable/段/Delta ≤S 归并仅解目标列，复用 P131/P134 版本归并与 P86②/P91 列提取基建）→ 事务长读窗输出端接线（3 列输出 + 谓词字段核验）。**流程**：干净 110 万单点基线 → demo（快照整行 vs 快照投影列 A/B）→ 内核 + 单测 → 探针复测。验收：#77 clean 110 万 mean 从 ~5.5s（P137）→ ≤1.5s（≥3×）、p99 抖动收敛；MySQL 对照 147ms | 立项（2026-09-06，demo-first）。**干净基线 ✅（2026-09-06，db-wide-scc-p140 wide-load 74s 立即跑，P139-c binary）：#77 mean 3316ms / p50 1848 / p99 8069（MySQL 147/145/160 → ≈22.5×）**——P139 系列后较 P137 clean（5486/1933/20767）已收敛（p99 20s→8s，窗口非 pk_range fields 路径受益有限但整体更稳）。**demo ✅（2026-09-06，src/demo/p140-snapshot-projection，gitignored，100k×25 列 wide-pax 配置 位图+per-CPU release）**：热态（全留 memtable）A_snap 0.43 / A_latest 0.33 / B_fields 2.62 µs/row——≤S 归并增量仅 +0.09（批量快照读全热 ≈ 免费）；SST 冷态（一次 flush_primary）A_latest **0.38**（get_many 按块批量点查、raw 字节不解码）/ B_fields 3.00（字段提取逐行 JSON parse 主导，整行 vs 3 列 **非解码差**）→ **列解码不是 #77 主成本**；而 A_snap = `batch_get_at` 逐 docid `get_bytes_at`（SST 冷段逐键 ≤S 点查、无 get_many 批量分组/无 HotCache 回填）**39.2 µs/row ≈ #77 33µs/row 同量级** → **#77 主成本 = SST 上快照 ≤S 读取机制本身（scan_range_txn/scan_range_at 逐行 ≤S 折叠 + 冷段点查）**。**内核取向修正**：`scan_range_txn_fields` = **范围迭代复用（一次 k-way merge，scan_range_at 同源）+ ≤S 折叠 + 目标列投影 三合一**（非仅"≤S 归并只解目标列"）；触发项：`batch_get_at` 需上 get_many 式按块批量点查（P135 superset 冷库回表退化面 39µs/row，get_many 0.38µs/row 证明可拉回 ~100×）。**内核 ✅（2026-09-06，P140）**：`Engine::scan_range_txn_fields(start,end,txn,fields)`——基表 `primary.scan_stream_at(S, project=Some(fields))`（一次 k-way 归并，PAX 只解目标列）+ Delta ≤S 白名单折叠（`fold_with_overrides_fields`）+ 位图仅 RC/当前视图剔除 + 自写覆盖/事务删除/新 docid 并入（与 `scan_range_txn` 尾部逐条一致，契约=返回行含 `fields` 各列）；接线事务 BETWEEN 窗：`plain_field_projection`（列清单仅 id+简单顶层字段、无 doc/嵌套路径/表达式且 ≥1 字段列 → 下推，否则整行回退）——`SELECT id,k,amount WHERE id BETWEEN`（#77 形态）免整行 25 列解码/组装。单测 p140_scan_range_txn_fields_projection_matches_full（mem/SST 双态 × RR ≤S / RC 合成 / 自写覆盖 / 事务删除 / 新 docid 并入，行集+目标列值 == scan_range_txn）；lib 793 全绿（seqlock flaky 复绿）。**复测 ✅（2026-09-06，同库 clean 重装 110 万后 P140 binary）：#77 mean 2459ms / p50 750 / p99 9674（首轮 2500.9/924/7049）**——warm 迭代 p50 1848→750-924（**2-2.5×**，投影下推收益实锤），mean 3316→2459（1.35×）；离群 7-10s = **冷首触 100MB 随机窗 IO + 装载后 L0/compaction 抖动**（与投影无关，基线同型 p99 8s）；验收 mean ≤1.5s 未达标 → 残余归 **IO/read-amp 族**（冷窗 IO 预取 / colstore #79-80 / compaction 收敛），非 P140 投影作用域；`batch_get_at` 按块批量触发项仍挂 superset 回表。**P140 收口** |
| **P145 批量快照取行（batch_get_at 按块分组）✅ 内核（2026-09-06）** | 收 P135/P138 触发项：`Engine::batch_get_at` 原逐 docid `primary.get_bytes_at`（SST 冷库 ~39µs/docid 点查）。内核 = CF `get_many_at(docids, S)`——语义与逐 `get_bytes_at` 完全一致（MemTable 多版本 ≤S + 全层 max seq ≤S、≤S Tombstone→None 短路、段 min_seq 整段剪枝）+ **逐 SST 按块分组**（同块只读/解码一次、块缓存复用，`scan_block_for_key_at(≤S)` 逐目标解析）；引擎 `batch_get_at` 主数据步改调之（Delta ≤S 折叠、不入 HotCache、快照计数埋点不变）。单测 p145_batch_get_at_grouped_matches_get_at（flush 落盘 × S0/S1 视图 × patch/delete/未写 docid，批量==逐 get_at）；lib 795 全绿。**收尾 A/B ✅（src/demo/p145-batch-get-at，直连 b32 库 100k 窗）**：冷 12.2µs/doc、热 8.8µs/doc（旧逐 docid batch_get_at @4KB = 39.2µs/doc → **3.2×**，收 P135/P138 触发项）；**实现要点**：块只解码一次（decode_data_block）+ 行集内 ≤S 选择——首版逐目标 `scan_block_for_key_at` 每调重解码整块（100k×32KB=解码放大，冷 143µs/doc 反效）→ 改后正确 |

### 事务聚合权威 + Estimate 数量级接口立项（2026-09-06，用户评估立项）

| 项 | 内容 | 状态 |
|---|---|---|
| **P141 事务内聚合 MVCC 权威版（A1）** | 缺口：RR 显式事务内 `COUNT(*)`/`COUNT(WHERE)`/`COUNT(DISTINCT)`/`GROUP BY` 聚合目前**受限（无 WHERE 1064）或最新口径**，与 RR 快照视图不自洽 = 语义缺陷。补齐（复用 P134/P135 基建）：`COUNT(*)` = 快照 ≤S 可见行数（scan_range_txn/scan_range_at 计数、不物化整行；**无 O(1) 捷径**——并发删需 ≤S 判定，正确但 O(窗) 为预期非缺陷）；`COUNT(WHERE)/DISTINCT/GROUP BY` = 候选 superset（倒排 posting 只增不删 = ever-match）+ `batch_get_at(S)` 逐行判活复核（P135 superset/P136 prune 现成）。附带：FOR QUICK 落点 = **默认权威、加词降级 O(1) 近似**的对照基线 | 立项（2026-09-06）。**接线完成**（2026-09-07：`src/server/command/txn_agg.rs` 挂载；`txn_select` 顶部 parse_select 聚合拦截 → 行源权威扫描——快照窗口 / FOR UPDATE 当前读 / sqlish 候选 + 同事务写自插 docid 窗口抬界；复用 group_by.rs `finalize_groups`/aggregate.rs `distinct_key_of`，空集数值聚合对齐 SQL NULL；server::tests 73 + sql::tests 67 回归绿）。✅ 全部完成（2026-09-07 A 批 A1-1~A1-3）：rr 探针双端对拍全绿（A1-1，含修复 txn DML BETWEEN 窗口 + JSON 类型化赋值）+ FOR QUICK 对照基线（A1-2，权威 ~5.2s/条 vs 加词 O(1) 快路 0.1ms-7.5ms；Ex-9.1 单等值快路缺 live∩ 高估墓碑 +53288）+ 内核确认（A1-3，sqlish 兜底与 wm>2^48 混合库分支可达性单测，含修复兜底 cond 尾截断 GROUP BY/HAVING，commit 917171c） |
| **P142 Estimate 数量级接口（A2）** | **独立非 SQL 入口**（HTTP `?estimate=true` / 独立命令 / /stats），契约 = "约 N 行（approx）"（当前已提交视图数量级），明确标注近似——与 MySQL 语义天然脱钩，**不进 SQL 关键字**。实现：引擎读锁内一次 `Engine::count_all_docs()`（全库）/`Engine::count_docs_range(start,end)`（表窗口 = docid 高 16 位表号；**单 Mutex 内 rank 差值已现成**，零 MVCC/锁/LSM 改动），返回标 approx；带条件估计（≈ COUNT WHERE f='v'）= bitmap_fields 内存位图或倒排 posting∩live（仍当前视图；换值旧值残留可接受——近似语义不修）。边界：live 懒建首调全键 keys-only 尖刺（open 后**后台预热**或接口标注首次慢）；两次调用数值不同是特性非 bug | 立项（2026-09-06）。✅ A 批完成（2026-09-07 A2-1~A2-4，commits 37fe410/5716dcb/93938cc/fb5a4dc）：HTTP `GET /estimate` 接线（`approx:true` + 数量级标注 + `field/value` + `range` 窗口 + 400 形态）+ 单测 + README 示例 + 500k 量级验证（全库/窗口/条件 estimate 与权威数量级对齐） |
...
| **M-7 SQL 类型化多字段 UPDATE（接线 engine.patch）** | engine.patch 支持多字段/JSON 类型化/null 删字段/Delta 合并，仅 HTTP/CLI patch 入口；SQL UPDATE 单字段且值一律字符串化——接线类型化 UPDATE = UPDATE 决策行候选①。**2026-09-07 治理核实（缺口收窄）**：P131（2026-09-06）已接 SQL 单非索引列 `SET col=字面量` → patch_batch（值类型化 + 免整行重写）；**多字段/JSON 类型化/null 删字段的 SQL 形态仍未接**，待评估 | 待评估（随 UPDATE 候选①） |
