...
- **状态**：✅ 同步模型部分完成（97e3586：预处理读语句走读锁 + 连接线程 512KB 小栈，478 全绿）；
  10k 连接 / 85 万 QPS 目标需**异步协程运行时**（M6 级改造，同步模型天花板）
- **再启条件**：异步运行时（tokio/async-std）接入网络层——每连接不再占 OS 线程，
  P99 高连接数下不飙升（design 9.5：10k 长连接 P99 +5% vs 同步 +300%）
