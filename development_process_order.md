...
| J | 倒排段 GC 后台化 | P2 | ✅ 完成 | b76dd40（7.73）：后台线程周期触发替代显式调用——InvertedIndex next_seg_id→AtomicU64、flush_segment/gc 改 &self + mutate 互斥锁（与写路径 flush 序列化 Manifest/删段文件，demo 确定性复现无锁丢失更新）；查询读段 NotFound 跳过（后台 GC 与查询并发）；Engine inverted Arc 化 + inverted_gc_pending 信号；mysql 后台 GC worker（信号+10 分钟兜底，不阻塞查询）；485 全绿 |
| K | fulltext 大 posting 反序列化优化 | P2 | ✅ 完成 | ed2588d（7.74）：段格式 v2→v3 posting 分块布局（容器头索引 + 独立容器字节，值存完整 docid 容器级对齐）；search_paged/doc_count 走惰性游标 k-way merge（分页只解码窗口容器、COUNT 精确跨段去重）；旧段 v2 兼容；实测 1600 万 posting 近页 x211、COUNT x4491、全量解码持平、数据 +0.1%；488 全绿 |
...
| AF | SQL 能力补全：ORDER BY / GROUP BY 系列 | P0 | 🚧 #1~#4 完成（2026-09-03） | 用户给定开发顺序（按价值/成本）：① ORDER BY 单字段+LIMIT（✅ da4c08a）→ ② GROUP BY 单字段+COUNT/SUM（✅ 9f81ec4）→ ③ ORDER BY 多字段（✅ 585cdeb）→ ④ GROUP BY 多字段+常用聚合（✅ 本期）→ ⑤ GROUP BY+HAVING → ⑥ 倒排索引加速 GROUP BY（Ex-9.3）；每项配套 demo/单测/文档后推进下一项 |
...
- **#4 GROUP BY 多字段 + 常用聚合（✅ 已完成 2026-09-03）**：`group_by` 单字段扩为多字段
  Vec（逗号列表、去重校验）；聚合白名单放开 AVG/MIN/MAX；`execute_group_by` 组键升级为
  复合键（Vec<GroupKey>，level 序），每组每聚合列独立 AggState（count/n_num/sum/min/max，
  消除 AF#2 多 SUM 聚合共享累积器隐患）；结果集分组列 = **选中普通列**（未选中分组字段
  仍参与分组但不出现在结果集，对齐 MySQL）；组行排序优先级 = ORDER BY 序列（各键须为
  分组字段）→ 剩余 level 升序补尾（可对次层键 DESC）；聚合输出 COUNT/SUM/AVG/MIN/MAX
  （空数值集 → NULL）；mysql.rs 渲染升级多分组列/DOUBLE 列型（AVG 小数）;
  - 测试 +2 并改写 AF#2 拒绝矩阵（多字段/AVG 由拒变允、重复字段/非分组列仍拒），
    586 全绿；wire 冒烟多字段+AVG 小数+次层排序+子集列断言通过；
- **待推进 #5~#6**：GROUP BY + HAVING → 倒排加速 GROUP BY（Ex-9.3）。
