...
| L | Compaction 智能调度（合并冷却 + 动态窗口 + 倒排阈值） | P1 | 🔄 优先实现 | ①合并冷却：compact 输出段冷却 N 轮不参与合并（防刚合并又合并，写放大 -10~20%）；②动态窗口：l0_stall_threshold 8~16 随写压力浮动（滞回防振荡）；③倒排刷盘阈值 100 万→500 万 term 对参数化（段数 -83%，查询段遍历 -83%）——风险均低可控（已分析） |
