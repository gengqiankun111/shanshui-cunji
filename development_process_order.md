...
| AD | 10 亿库扩展阶段 A：全局 docid 分配器（分片前缀 `shard_id<<40\|local_id`） | P0 | 📋 规划完成（design-10b-extension.md）待开发 | 10 亿库前提：docid 路由 O(1)、分片内自增无集中瓶颈、扩容归属不变；→ 分片构建工具 → 10 分片 10 亿构建（目标点查 10 万+ QPS / 写入 20 万+ rows/s / 构建 ≤60 分钟） |
