...
| 项 | 内容 | 状态 |
|---|---|---|
| **P134 事务读快照语义收口（正确性，先行）** | 两缺口：① `txn_select_by_predicate` 字段谓词候选来自最新态（回表 `batch_get` 删除位图剔除快照后删行 → 事务内重复读消失，与点查 get_at 见旧值不自洽）；② `scan_range_txn` 快照扫描仍按删除位图剔行（位图无 seq）。修法：候选/扫描不做"最新态删除"预过滤（posting 只加不删 ⇒ 候选含历史 docid），可见性统一交快照裁决（txn_get / scan_range_at tombstone seq）；③ 核对快照锚定时机（begin 注册 vs MySQL 首条一致性读）；顺带 mem `Vec<u64>` flush 前排序（闭合 merge_distinct 升序前提） | 待排（1.5~2 天） |
