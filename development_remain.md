...
| **P135 倒排快照读统一接线（snapshot-first）** | 无 posting 版本化。`Engine::batch_get_at(docids,S)` **✅ 内核已落**（语义=逐 get_at：跳过位图、tombstone seq 裁决、Delta ≤S 折叠单次窗口；**不入 HotCache**——快照视图≠最新态；读.rs + 单测 p135_batch_get_at_matches_get_at 位图关/开×删/换值/patch 跨快照/空集/跨表）；**接线待做**：事务谓词候选复核、非事务倒排消费端（eval_cond posting 命中 / collect_limited_rows 回表）、区间/组合索引同构；autocommit S=语句级全局 seq 选型（保留位图快路径 or 统一快照读 demo 定） | demo ✅（2026-09-06，src/demo/p135-batch-get-at）+ **内核 batch_get_at ✅ + 单测（2026-09-06）**；全量 lib 785 过（seqlock flaky 复绿）；**接线待做** |
