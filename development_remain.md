...
| **P136 快照活跃预过滤（C，可选小项）** | live_docids v2 = docid 级 (add_seq,del_seq) → snapshot_live(S) 集合级剔除"S 前已删"候选（免空回表）；换值无 del、仍需字段复核（明确局限）；与 active_snapshots/mvcc_keep_floor 联动防堆积 | 待排（依赖 P135） |
