...
| **P135 倒排快照读统一接线（snapshot-first）** | 无 posting 版本化。`Engine::batch_get_at(docids,S)` ✅（语义=逐 get_at：跳过位图、tombstone seq 裁决、Delta ≤S 折叠单次窗口；不入 HotCache）；**接线：txn 谓词 superset ✅（2026-09-06）**——RR 字段谓词 WHEN 纯倒排等值 AND 链 → `inverted_eq_superset`（posting 只加不删 = ever-match 超集）∩窗口 → `batch_get_at(S)` 复核 + `doc_matches_where`（恢复索引加速、免 P134 全窗扫；RR 正确：快照后删/换值行仍在 posting→≤S 裁决）；复杂/空 posting/含未提交写 → 全窗扫兜底。**接线待做**：非事务倒排消费端（eval_cond posting 命中 / collect_limited_rows 回表）、区间/组合索引同构；autocommit S=语句级全局 seq 选型 | demo ✅ + 内核 batch_get_at ✅ + txn 谓词 superset ✅（2026-09-06）；3 新单测/集成（batch==get_at 引擎测、superset RR 端到端 delete/update/SUM、superset gating 判定）；全量 lib 787 过（seqlock flaky 复绿）；**非事务倒排消费端/autocommit S 接线待做** |
