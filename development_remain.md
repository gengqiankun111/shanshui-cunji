...
| **P135 倒排快照读统一接线（snapshot-first，demo 先行）** | 无 posting 版本化。新 API `Engine::batch_get_at(docids,S)`（语义=逐 get_at：跳过位图、tombstone seq 裁决、Delta 折叠 ≤S）+ 接线：事务谓词候选复核、非事务倒排消费端（eval_cond posting 命中 / collect_limited_rows 回表）、区间/组合索引同构；autocommit S=语句级全局 seq 选型（保留位图快路径 or 统一快照读 demo 定） | 待排（依赖 P134） |
