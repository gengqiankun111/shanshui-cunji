...
| **P145 批量快照取行（batch_get_at 按块分组）✅ 内核（2026-09-06）** | 收 P135/P138 触发项：`Engine::batch_get_at` 原逐 docid `primary.get_bytes_at`（SST 冷库 ~39µs/docid 点查）。内核 = CF `get_many_at(docids, S)`——语义与逐 `get_bytes_at` 完全一致（MemTable 多版本 ≤S + 全层 max seq ≤S、≤S Tombstone→None 短路、段 min_seq 整段剪枝）+ **逐 SST 按块分组**（同块只读/解码一次、块缓存复用，`scan_block_for_key_at(≤S)` 逐目标解析）；引擎 `batch_get_at` 主数据步改调之（Delta ≤S 折叠、不入 HotCache、快照计数埋点不变）。单测 p145_batch_get_at_grouped_matches_get_at（flush 落盘 × S0/S1 视图 × patch/delete/未写 docid，批量==逐 get_at）；lib 795 全绿。**性能复测待办**：superset 冷库回表路径 A/B（预计 39µs/docid → 块级顺序读量级） |
