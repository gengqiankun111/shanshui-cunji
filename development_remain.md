...
| **Bloom 读路径核查（2026-09-05，audit 结论见 research/bloom_readpath_audit.md）** | **布隆已全链实现并接线（非缺失）**：v5 分区布隆（每数据块一个，fpr 默认 0.01）+ v3/v4 legacy 兼容；flush 与 compaction 合并输出均由 SstWriter 重建新布隆（merge.rs 传 bloom_fpr）；读路径（当前读/快照读/批量点查）均在读块前调用目标块 `maybe_contains`（read.rs 351/361/406/454 等）。886ms max 长尾非布隆缺失（冷 IO/首触 + L0 多段线性放大更可能）。**残余缺口（新排期）**：①布隆跳过/命中计数 metrics（配 L0 4/8/12/16 + miss 点查压测验证收益）；②每 SST bloom/索引元数据字节纳入 memory_report；③MVCC 活跃快照数/最长存活 metric + 超时告警 + 最大存活强制回收（防长快照阻塞 compaction，关联 txn_long_read 长尾）；④886ms 尖峰冷热对照实验 | audit ✅（文档）；①~④ 待排 |
