...
| Ex-8.13 倒排后台 IO 预算共享（P3） | **设计已出（同上 research 文档 §2/§5-step1）：抽 io_budget 共享模块 + 倒排 seg 写记账（紧急刷段不受限、GC/空闲刷段记账节流）；demo A/B（前台写 + inverted_gc，p99 退化≤5%）随内核切片实现** | 设计 ✅ / 内核待做 |
