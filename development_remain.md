...
| **P135 倒排快照读统一接线（snapshot-first）** | 无 posting 版本化。新 API `Engine::batch_get_at(docids,S)`（语义=逐 get_at：跳过位图、tombstone seq 裁决、Delta 折叠 ≤S）+ 接线：事务谓词候选复核、非事务倒排消费端（eval_cond posting 命中 / collect_limited_rows 回表）、区间/组合索引同构；autocommit S=语句级全局 seq 选型（保留位图快路径 or 统一快照读 demo 定） | demo ✅（2026-09-06，src/demo/p135-batch-get-at，gitignored）：get_at 语义矩阵 20/20（位图关/开 × 快照后删/前删/换值/复活/多版本/跨表/不存在）；批量潜力 A/B（debug warm 4000 docid）逐 get_at 乱序 4.69 / 升序 4.44 / latest batch_get 2.92 µs-op（batch ~1.6×，升序 1.06×；冷态/大库差距更大）。**内核 `Engine::batch_get_at`（CF get_many_at：升序批取 + 块局部性 + seq 单点换算，复刻 P2-D batch_get 骨架）待做** |
