Redis 集成部署指南（Redis Integration Guide）
版本：v1.0
适用版本：novosdb >= v0.2（分布式阶段）
前置条件：已部署 Redis 5.0+（单机 / 哨兵 / 集群）
关联文档：design.md | development.md

1. 设计哲学：Redis 是“读加速器”，不是“数据源”
角色	说明
Novosdb	唯一的真理源（Source of Truth）。所有写入必须落盘（WAL + SST）。
Redis	分布式热读缓存（L2 Cache）。仅在 Novosdb 之上加速查询，绝不作为写入缓冲。
同步方向	单向：Novosdb → Redis（回填 / 失效）。绝不支持 Redis → Novosdb 反向写入。
1.1 黄金法则（必须遵守）
先落盘，后缓存：写入顺序必须为 Novosdb WAL → 返回 ACK → 异步失效/更新 Redis。严禁先写 Redis 再异步写 Novosdb。

缓存只是副本：Redis 数据可随时丢失或清空，业务不应受影响（仅降级为较慢的磁盘读取）。

不做双向同步：绝不实现 Redis → Novosdb 的数据同步，防止一致性灾难。

2. 部署模式（三种典型方案）
2.1 模式一：单机 Novosdb + 本地 HotCache（默认，无外部 Redis）
适用场景：数据量 < 2 亿，QPS < 5 万，无需跨节点共享缓存。

配置：config.toml 中 [cache.external] enabled = false（默认）。

缓存层次：Client → Novosdb HotCache（进程内）→ Disk。

延迟：热点命中 0.1~0.3ms。

2.2 模式二：集群 Novosdb + Redis 旁路缓存（推荐）
适用场景：数据量 > 2 亿，热点集中，QPS > 10 万，多节点共享热数据。

配置：启用 [cache.external]，Redis 作为 L2 分布式缓存。

缓存层次：Client → Gateway → Redis（L2）→ Novosdb HotCache（L1）→ Disk。

延迟：Redis 命中 0.1~0.3ms；穿透 Novosdb 0.5~2ms。
...
收益：拦截 90% 以上的无效查询，保护 Novosdb 不被穿透。
...
更新需要额外读一次 Novosdb（增加 IO），且可能出现并发写乱序。
...
场景	保障措施	一致性级别
正常写入	先写 Novosdb（权威源），再删 Redis 缓存	强一致（读请求下次回填最新值）
并发写入同一 key	Novosdb 自身有版本号/时间戳，最后写入者胜出；Redis 缓存被多次删除，最终是最新值	最终一致（毫秒级收敛）
Redis 不可用	熔断器打开，直接降级读 Novosdb，不返回旧数据	降级一致（读权威源）
缓存穿透（回填期间）	多个请求同时 miss 时，使用 互斥锁（Mutex），仅一个请求查 Novosdb	防击穿
缓存雪崩	TTL 加随机抖动（TTL = 300 + rand(60)）	防雪崩
6. 故障处理与降级策略
故障类型	检测方式	处理动作	用户影响
Redis 连接超时	timeout_ms 超时触发	立即返回错误，不阻塞，降级读 Novosdb	延迟从 0.1ms 升至 0.5~2ms
Redis 节点宕机	连接池检测到断连	熔断器（Circuit Breaker）打开，后续请求直接透传 Novosdb；定时探测恢复	同超时降级
...
# 手动清空指定 key 的缓存
novosdb admin cache-evict --key docid:1001

# 手动预热（将热点 key 提前加载到 Redis）
novosdb admin cache-warm --filter 'status="active"' --limit 10000
7.2 监控指标（接入 Prometheus）
指标名	类型	说明
novosdb_cache_redis_hits_total	Counter	Redis 命中次数
novosdb_cache_redis_misses_total	Counter	Redis 未命中次数
novosdb_cache_redis_errors_total	Counter	Redis 操作失败次数
novosdb_cache_redis_latency_seconds	Histogram	Redis 操作延迟分布
novosdb_cache_redis_connections_active	Gauge	当前活跃连接数
8. 性能基准（参考值）
8.1 各缓存层级延迟对比（P95）
层级	介质	P95 延迟	说明
L1：进程内 HotCache	内存（本地）	0.05~0.1ms	Novosdb 自带，无需外部依赖
L2：Redis 分布式缓存	内存（网络）	0.1~0.3ms	本集成方案，跨节点共享
L3：Novosdb 冷查询	磁盘 + BlockCache	0.5~2ms	缓存未命中时降级至此
8.2 吞吐能力（单网关，16核）
场景	QPS	说明
Redis 命中	~150,000 QPS	受限于网络和 Redis 单线程性能
Redis 未命中（降级 Novosdb）	~50,000 QPS	受限于 Novosdb 查询能力
...
❌ 绝对禁止
绝对禁止“先写 Redis 再异步写 Novosdb”：如果 Novosdb 写入失败，Redis 中会留下脏数据。

绝对禁止“双向同步”：Redis → Novosdb 的反向同步会导致不可控的环形更新。
...
🚨 常见误区澄清
误区	真相
“Redis 挂了业务就不可用”	❌ 熔断器会自动降级读 Novosdb，业务仅变慢，不会不可用。
...
阶段 3	Redis Streams 变更订阅（CDC）	将 Novosdb 的写入事件（WAL）通过 Redis Streams 广播给订阅者
11. 附录：快速配置模板
A. 最小配置（单机 Redis）
toml
[cache.external]
...
12. 总结
维度	结论
定位	Redis 是 Novosdb 的 L2 分布式热读缓存，不是数据源。
同步方向	单向：Novosdb → Redis（回填 + 失效）。
写入顺序	先 Novosdb，后 Redis（失效），保证数据一致性。
读取策略	Cache-Aside：先查 Redis，未命中则查 Novosdb 并回填。
可用性	Redis 不可用时，熔断器自动降级读 Novosdb，业务仍可用。
一致性	默认最终一致性；可选“双删”提升至近似强一致。
一句话概括：

“Novosdb 管持久化，Redis 管读加速——各司其职，协同工作，这才是现代云原生架构的正确姿势。”