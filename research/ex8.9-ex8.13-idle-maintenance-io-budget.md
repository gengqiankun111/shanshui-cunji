...
**交变验收补全 + 4/4 复验（2026-09-06，P133）**：原「前台 busy 耗时」为整轮 burst 代理（写锁内测），
追加**忙窗读 p99 探针**（`ex89_ab_busy_read_p99_no_degradation`：无信号 L0 积压 5 段 → A idle_run 收敛
vs B 滞留 → 命中+miss 点读逐操作计时，测前 400 warmup 避换段冷 IO）：
| 场景 | B 旧固定节奏 | A 空闲感知 | 判据 |
|---|---|---|---|
| 忙窗读 p50/p99（L0 1→5 积压后） | 2.2 / 17.1µs（积压滞留 5 段） | 2.0 / 4.6µs（idle_run 收敛 5→0） | A 收敛态 ≤ 自身干净基态 ×1.8 且 ≤ B ×1.2 ✅ |
4/4 `--ignored` 23.5s 全过；全量 lib 783 绿（+1 ignored 计入，非运行态）。
