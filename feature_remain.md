...
| 功能 | 模块 | 说明 |
|---|---|---|
| posting LRU 双区热点化（Ex-8.8，✅ 8f70b3e） | 倒排/缓存 | 单 LRU→protected+probation 双区（60/40，容量参数化）；命中提升免被冷突发逐出；写路径清空不变（561 全绿） |
| 段级 min seq 快照剪枝（Ex-8.6，✅ 29be7b1） | 快照读 | get_bytes_at/scan_stream_at 整段跳过（惰性 keys-only 推导+记忆，重启自动重建；与 Ex-8.2 双维剪枝，560 全绿） |
