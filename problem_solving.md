...
- 决策接口 → 实测回填（2026-09-05）：demo `src/demo/l0-bloom-scale`（release，偶数 8000 行交错写
  ...
  **决策（2026-09-05 用户修正）**：per-table L0 优先压实**立项高优先**——本压测仅证单表 miss
   ...
   （该表段数达阈只压该表，全局兜底，见 development_remain P129 排期行）；分层 fpr 调优对纯 miss
   无意义（真 miss 无假阳性），仅命中场景假阳性读块受益 → 需另测命中档方可立项，暂缓；布隆反序列化
   缓存量级 ≈0.2µs/段不单独立项。读放大压测范围仅为单表 miss 斜率，不覆盖多表计数/写放大面。
